...
2. Are there fees for decrypting/encrypting?
...
3. Can I deploy my signing condition without including it in my codebase?
...
4. If I call saveSigningCondition() twice with the exact same conditions, do two copies get created and stored by nodes?
...
5. If I already have a connect/sign mechanism in place, can I just create my own authSig object and pass it to saveSigningCondition() instead of using checkAndSignAuthMessage()?
...
6. Can more than one condition be added for access control?

Yes! See https://developer.litprotocol.com/docs/AccessControlConditions/booleanLogic for examples.

7. Can I delete or edit a published resourceId?
...
8. Are encryptedSymmetricKey and encryptedString unique to a user or global for a piece of content that we've encrypted?
...
9. I guess the real tricky part to get right is tuning things so that the attack I asked about before - where one operator runs enough nodes to gather sufficient fragments to decrypt stuff - is sufficiently difficult and costly to execute so as to be impractical.  That still seems like the most likely attack vector if things aren't designed just right.  So long as that attack remains impractical then the system is pretty robust
